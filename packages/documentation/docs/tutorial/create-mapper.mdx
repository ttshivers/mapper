...
First step is to install `@ttshivers/automapper-*` via `npm` or `yarn`

```shell
npm i @ttshivers/automapper-core @ttshivers/automapper-classes reflect-metadata
```

```shell
yarn add @ttshivers/automapper-core @ttshivers/automapper-classes reflect-metadata
```

:::info

-   We are using `@ttshivers/automapper-classes` for this tutorial. Check [Installation](../getting-started/installation) for more details.
-   The rest of the tutorial will apply to `@ttshivers/automapper-classes` only. If you're using `@ttshivers/automapper-pojos`, please check [POJOs Strategy](../strategies/pojos)
...
With `@ttshivers/automapper-classes`, you will use `@AutoMap()` decorator to do so. Let's bring back the models
