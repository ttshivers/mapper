...
`@ttshivers/automapper-nestjs` is the official integration for [NestJS](https://nestjs.com).

## Installation

```shell
npm i @ttshivers/automapper-core @ttshivers/automapper-nestjs
```

```shell
yarn add @ttshivers/automapper-core @ttshivers/automapper-nestjs
```

### Strategy

Recommendation is to use `@ttshivers/automapper-classes` in a NestJS application.
...
import { createMap } from '@ttshivers/automapper-core';
...
`@ttshivers/automapper-nestjs` provides `MapInterceptor`. In cases where you do not care about annotating the correct return type for a **Controller#method** and want your **Service** to be a little cleaner, you can utilize the `MapInterceptor` to execute the mapping.

```ts
import { UseInterceptors } from '@nestjs/common';
import { MapInterceptor } from '@ttshivers/automapper-nestjs';
...
`@ttshivers/automapper-nestjs` provides `MapPipe`. When you want to transform the incoming request body before it gets to the route handler, you can utilize `MapPipe` to achieve this behavior

```ts
import { MapPipe } from '@ttshivers/automapper-nestjs';
