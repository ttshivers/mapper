...
`@ttshivers/automapper-pojos` is an official strategy that works with TS Interface-based projects.

## Installation

```shell
npm i @ttshivers/automapper-core @ttshivers/automapper-pojos
```

```shell
yarn add @ttshivers/automapper-core @ttshivers/automapper-pojos
```

## Usage

Different from `@ttshivers/automapper-classes`, `@ttshivers/automapper-pojos` exports a singleton `PojosMetadataMap` so we can provide the metadata for `pojos`.
...
As seen above, `PojosMetadataMap` is a way that `@ttshivers/automapper-pojos` use to keep track of the Interface's metadata.
...
Same as `@ttshivers/automapper-classes`, we still need to explicitly supply the Enum type
...
Same as `@ttshivers/automapper-classes`, we still need to explicitly supply the Array type
...
Same as `@ttshivers/automapper-classes`, we can also have circular dependency with Interfaces.
